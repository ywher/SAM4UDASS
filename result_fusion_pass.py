...
import tqdm
from config import config_pass as config
...
if __name__ == '__main__':
    ...
    for i in index_range:
        ...
        if not no_gt:
            ...
            gt_color = np.zeros((uda_pred.shape[0], uda_pred.shape[1], 3), dtype=np.uint8)
            mask_color = np.zeros((uda_pred.shape[0], uda_pred.shape[1], 3), dtype=np.uint8)
        ...
        error_0, error_1, error_2, error_3 = black_img, black_img, black_img, black_img
